...
public class ListaCD<T> implements Iterable<T> {
    ...
    public void insertarTodo(ListaCD<T> temp) {
        ...
        temp.cabeza.setSig(null);
        temp.cabeza.setAnt(null);

    }
    
    public void concatenarListas(ListaCD<T> temp, int posicion) {

        NodoD<T> cola = temp.cabeza.getAnt();
  
        NodoD<T> inicio = this.getPos(posicion);
        
        cola.setSig(inicio.getSig());
        inicio.getSig().setAnt(cola);
        
        inicio.setSig(temp.cabeza.getSig());
        temp.cabeza.getSig().setAnt(inicio);
    ...
    public void addAllSet(Set<T> temp)
    {
    
        for(T t : temp)
        {
        
            this.insertarFinal(t);
